...
abstract class RestaurantsByIDController extends GetxController {
  viewrestaurants();
}

class RestaurantsByIDControllerImp extends RestaurantsByIDController {
  ...
  viewrestaurants() async {
    ...
    update();
    var response = await restaurantsData.getdataID(
      myServices.sharedPreferences.getInt("restaurants_id").toString(),
    );
    if (StatusRequest.success == handlingData(response)) {
      ...
      if (mapData["status"] == true) {
        ...
        calculateDistance(myServices.sharedPreferences.getDouble('lat'),myServices.sharedPreferences.getDouble('long'),mapData['data'][0]['restaurants_lat'],mapData['data'][0]['restaurants_long']);
