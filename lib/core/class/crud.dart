...
class Crud {
  ...
  postFileAndtwoFiles(String linkUrl, Map data, Map<String, String> headers, File file, File idFile) async {
    ...
    var multiFile = http.MultipartFile('files', streamData, fileLength, filename: basename(file.path));
    var idMultiFile = http.MultipartFile('files2', idStreamData, idFileLength, filename: basename(idFile.path));
  ...
  postFile2AndData(
    ...
    int fileLength = await file.length();
    var streamData = http.ByteStream(file.openRead());
    var multiFile = http.MultipartFile('files2', streamData, fileLength,
        filename: basename(file.path));
    request.files.add(multiFile);
    data.forEach((key, value) {
      request.fields[key] = value;
